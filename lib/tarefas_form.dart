...
class TarefasForm extends StatefulWidget {
  ...
  const TarefasForm({required this.state, required this.helper});
...
class _TarefasFormState extends State<TarefasForm> {
  final _dateController =
      TextEditingController(); // controla o estado do campo da data
  ...
  void initState() {
    super.initState();
    _tarefa =
        widget.state.tarefa ?? Tarefa(descricao: "", prazo: DateTime.now());
          ...
          children: [
            TextFormField(
              ...
              validator: (value) => (value ?? "").isEmpty
                  ? "Uma descrição para a tarefa deve ser informada."
                  : null, // 4- Validador do campo descrição
              onSaved: (value) => _tarefa.descricao =
                  value!, // 7- Recebendo o valor do campo e armazenando na propriedade do objeto que está recebendo os dados do formulário
            ),
            const SizedBox(
              height: 10,
            ),
            TextFormField(
              ...
              //05/10
              onSaved: (value) => _tarefa.prazo = dateFormat.parse(
                  value!), // 8 - armazenando a data no campo prazo da tarefa
            ),
            const SizedBox(
              height: 10,
            ),
            ElevatedButton(
                onPressed: () async {
                  ...
                  if (_formKey.currentState!.validate()) {
                    ...
                    print(
                        "Tarefa digitada: $_tarefa"); // 9- Aqui iremos enviar para o banco de dados
